...
function isPalindrome(str) {
  ...
  for (let i = 0; i < strArr.length / 2; i += 1) {
    if (strArr[i].toLowerCase() !== strArr.reverse()[i].toLowerCase()) {
      return false;
    }
  }
  return true;
...
function getCardId(value) {
  ...
  const result = [value.slice(0, value.length - 1), value.slice(-1)];
  if (result[0] in values) {
    return values[result[0]] + suits[result[1]];
  }
  return Number(result[0]) + suits[result[1]] - 1;
  // throw new Error('Not implemented');
}
